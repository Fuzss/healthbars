...
public class EntityRelationHelper {
    ...
    public static boolean showHealthBar(Entity entity, Minecraft client) {
        return entity instanceof LivingEntity livingEntity && !(entity instanceof ArmorStand) && (!entity.isInvisibleTo(
                client.player) || entity.isCurrentlyGlowing() || entity.isOnFire() ||
                entity instanceof Creeper && ((Creeper) entity).isPowered() // charged creeper
                || StreamSupport.stream(livingEntity.getAllSlots().spliterator(), false).anyMatch(
                itemStack -> !itemStack.isEmpty())) && entity != client.player && !entity.isSpectator();
    }
}
